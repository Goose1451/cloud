...
def colorAll(strip, color, wait_ms=200):
        for i in range(strip.numPixels()-5):
            strip.setPixelColor(i, color)
        strip.show()
        for z in range(SUN_LED_START, SUN_LED_START + SUN_LED_COUNT):
            for q in range(3):
			    for x in range(14, 19, 2):
				    strip.setPixelColorRGB(x+q, 255, 255, 0)
			    strip.show()
			    time.sleep(wait_ms/1000.0)
			    for x in range(14, 19, 2):
				    strip.setPixelColor(x+q, 0)
        #for x in range(SUN_LED_START, SUN_LED_START + SUN_LED_COUNT):
...
def getWeather():
    ...
    url = requests.get('https://query.yahooapis.com/v1/public/yql?q=select item.forecast from weather.forecast where woeid in (select woeid from geo.places(1) where text="portland, or")&format=json')
    global weather
